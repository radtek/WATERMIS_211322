...
namespace Common.DotNetUI
{
    public class ControlBindHelper
    {
        ...
        public static void BindComboBoxData(System.Windows.Forms.ComboBox comboBox, DataTable dt, string DisplayMember, string ValueMember)
        {
        ...
        public static void BindComboBoxData(System.Windows.Forms.ComboBox comboBox, DataTable dt, string DisplayMember, string ValueMember,bool AddNull)
        {
            if (AddNull)
            {
                DataTable dtn = new DataTable();
                dtn.Columns.Add(DisplayMember);
                dtn.Columns.Add(ValueMember);

                string[] strColumns = new string[] { DisplayMember, ValueMember };

                DataRow dr = dtn.NewRow();
                dr.ItemArray = new object[] { "全部", null };
                 dtn.Rows.Add(dr);

                 DataTable dttemp = dt.DefaultView.ToTable(true, strColumns);
                 foreach (DataRow dd in dttemp.Rows)
                 {
                     dtn.ImportRow(dd);
                 }
                
                BindComboBoxData(comboBox, dtn, DisplayMember, ValueMember);
            }
            else
            {
                BindComboBoxData(comboBox, dt, DisplayMember, ValueMember);
